...
function ipMedicalHistoryInfoDao(db) {
    this.db = db;
    ...
    this.findHospitals = async function() {
        var where = `select hospital_id   from ip_medical_history_info group by hospital_id`;
        return await this.db.query(where, []);
    }
}
module.exports = ipMedicalHistoryInfoDao;